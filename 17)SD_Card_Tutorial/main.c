...
void main(void){
    ...
    f_write(&file, deneme, sizeof(deneme), &bytesWritten);
    f_write(&file, deneme, sizeof(deneme), &bytesWritten);
    f_close(&file);
    f_mount(0,0);
    while(1);
...
void fat_init(void){
    ...
    while (errCode != FR_OK){                               //go until f_open returns FR_OK (function successful)
        ...
        errCode =f_mkdir("/berk");
        errCode =f_mkdir("/berk/deneme");

        errCode = f_open(&file, "/berk/deneme/deneme.txt", FA_CREATE_ALWAYS | FA_WRITE);
        if(errCode != FR_OK)
